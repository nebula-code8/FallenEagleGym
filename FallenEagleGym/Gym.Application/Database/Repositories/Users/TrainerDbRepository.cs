...
namespace Gym.Application.Database.Repositories.Users;

public class TrainerDbRepository: BaseRepository
{
     public Trainer? GetById(long id) {
        using IDbConnection connection = CreateConnection();
        using IDbCommand command = connection.CreateCommand();
        command.CommandText = "SELECT u.*, t.specialization, t.biography, t.license " 
                            + "FROM users u INNER JOIN trainer t ON u.id = t.id " 
    ...
    public List<Trainer> GetClientTrainers(long clientId)
    {
        ...
        using IDbCommand command = connection.CreateCommand();
        command.CommandText = @"SELECT u.*, t.specialization, t.biography, t.license " 
                             + "FROM users u INNER JOIN trainer t ON u.id = t.id INNER JOIN trainer_requests tr ON t.id = tr.trainer_id " 
    ...
    public List<Trainer> GetAllTrainersWithRatings()
    {
        ...
        command.CommandText = @"SELECT u.*, t.specialization, t.biography, t.license, COALESCE(AVG(tr.rating), 0) as AverageRating "
                             + "FROM users u INNER JOIN trainer t ON u.id = t.id LEFT JOIN trainer_ratings tr ON t.id = tr.trainer_id " 
                             + "WHERE u.role = @role "
                             + "GROUP BY u.id, t.id " 
    ...
    // TODO: fix this code so that licencese are loaded from the seperate database
    private static Trainer MapDbRowToTrainer(IDataRecord reader)
    {
        ...
        return new Trainer(Convert.ToInt64(reader["id"]), reader["name"].ToString(), reader["surname"].ToString(),
            ...
            reader["specialization"].ToString(), reader["biography"].ToString()
        );
    }
    
    // TODO: fix this so that it does not set average rating but rather it sets the List<Rating>
